...
export default function AdminAdRequests({
  ...
  const handleDecisionAd = (id, decision) => {
    ...
    api
      .post("/api/freetonadmin/ad", postData)
      .then((response) => {
  ...
  const handleDeleteAd = (id) => {
    ...
    api
      .post("/api/freetonadmin/delete/ad", postData)
      .then((response) => {
        setAdminAds(response.data.advertisements);
        showNotification("Успешно выполнено");
        setIsLoading(false);
  ...
  const handleDeleteChallenge = (id, challengeType) => {
    ...
    api
      .post("/api/freetonadmin/delete/challenge", postData)
      .then((response) => {
  ...
  const handleDecisionChallenge = (id, decision, challengeType) => {
    ...
    api
      .post("/api/freetonadmin/challenge", postData)
      .then((response) => {
        setChallenges(response.data);
        showNotification("Успешно выполнено");
        setIsLoading(false);
  ...
  useEffect(() => {
    ...
    const mergedArray = challenges.allSurfingChallenges.concat(
      challenges.allTelegramChallenges,
    );
  ...
  const renderAdminChallenges = () => {
    ...
    return sortedChallenges.map((challenge, index) => {
      if (challenge.timeOfExecution !== undefined) {
        return (
          <div
            className="challenge-moderation-container"
            key={challenge.id || index}
          >
            <div className="challenge-row sub">
              <div className="challenge-row-sub start">
                <div className="challenge-item-text challenge-name">
                  {challenge.name}
                </div>
                <div className="challenge-item-text challenge-description">
            ...
            <div className="moderarion-info-block">
              ...
              <div
                className="challenge-item-text"
                onClick={() => copyLink(challenge.link)}
              >
                ССЫЛКА: {challenge.link}
              </div>
              <div className="challenge-item-text">
                ПОВТОРЕНИЙ: {challenge.numberOfExecution}
              </div>
              <div className="challenge-item-text ">
                ЦЕНА ЗА ВСЁ:{" "}
                {challenge.price * (5 / 3) * challenge.numberOfExecution} TON
              </div>
            </div>
            {challenge.status === "moderation" ? (
              <div className="withdrawal-request-buttons">
                <button
                  ...
                  onClick={() =>
                    handleDecisionChallenge(challenge.id, "active", "surfing")
                  }
                  disabled={isLoading}
                >
                  ОДОБРИТЬ
                ...
                <button
                  ...
                  onClick={() =>
                    handleDecisionChallenge(challenge.id, "deny", "surfing")
                  }
                  disabled={isLoading}
                >
                  ОТКЛОНИТЬ
              ...
              <>
                <button
                  className="withdrawal-button delete"
                  onClick={() => handleDeleteChallenge(challenge.id, "surfing")}
                  disabled={isLoading}
                >
                  УДАЛИТЬ
                </button>
            ...
            <div className="challenge-row sub">
              ...
              <div className="challenge-row-sub end">
                <div className="challenge-time-container">
                  ...
                  <div className="challenge-item-text challenge-time-of-execution">
                    {getTypeMeaning(challenge.selectedType)}
                  </div>
                </div>
                <span className="challenge-item-payment">
                  <div className="challenge-item-text challege-price">
            ...
            {challenge.status === "moderation" ? (
              <div className="withdrawal-request-buttons">
                <button
                  className="withdrawal-button yes"
                  onClick={() =>
                    handleDecisionChallenge(challenge.id, "active", "telegram")
                  }
                  disabled={isLoading}
                >
                  ОДОБРИТЬ
                ...
                <button
                  className="withdrawal-button no"
                  onClick={() =>
                    handleDecisionChallenge(challenge.id, "deny", "telegram")
                  }
                  disabled={isLoading}
                >
                  ОТКЛОНИТЬ
              ...
              <>
                <button
                  className="withdrawal-button delete"
                  onClick={() =>
                    handleDeleteChallenge(challenge.id, "telegram")
                  }
                  disabled={isLoading}
                >
                  УДАЛИТЬ
      ...
      <div className="admin-ad-container" key={ad.id || index}>
        <div className="demo-ad-container">
          <div className="demo-ad-info-container">
            <div className="demo-add-info">Пакет: {ad.adPackageName}</div>
            ...
            <div className="demo-add-info">Ссылка: {ad.adLink}</div>
            {ad.status === "active" && (
              <div className="demo-add-info">
                Показ закончится:{" "}
                {calculateDeadline(
          ...
          <div className="advertising-container">
            <div className="add-container">
              <div className="add-text">{ad.adText}</div>
        ...
        <div className="admin-ad-manage-container">
          {ad.status === "moderation" ? (
            <div className="withdrawal-request-buttons">
              <button
                className="withdrawal-button yes"
                onClick={() => handleDecisionAd(ad.id, "active")}
                disabled={isLoading}
              >
                ОДОБРИТЬ
              </button>
              <button
                className="withdrawal-button no"
                onClick={() => handleDecisionAd(ad.id, "deny")}
                disabled={isLoading}
              >
                ОТКЛОНИТЬ
              </button>
  ...
  return (
    <div className="ad-challenges-container">
      <div className="ad-menu-button-container">
        <button
          className={`ad-menu-button ${adPage === "ads" ? "btn-active" : ""}`}
          onClick={() => handleMenuButtonClick("ads")}
        >
          РЕКЛАМА
        </button>
        <button
          className={`ad-menu-button ${adPage === "challenges" ? "btn-active" : ""}`}
          onClick={() => handleMenuButtonClick("challenges")}
        >
          ЗАДАНИЯ
        </button>
      </div>
      <div
        ...
        style={{ display: adPage === "ads" ? "block" : "none" }}
      >
        {renderAdminAds()}
      </div>
      <div
        className="admin-ads-list"
        style={{ display: adPage === "challenges" ? "block" : "none" }}
      >
