...
export default function AdminSettingsChallenges({
  surfingConfigs,
  setSurfingConfigs,
  telegramChallengesConfig,
  setTelegramChalengesConfig,
}) {
  ...
  useEffect(() => {
    if (
      ...
      FIELD_NAMES.forEach((f) => {
        valObj[f.field] = String(surfingConfigs[0][f.field]);
      });
      setInputs(valObj);
  ...
  useEffect(() => {
    if (
      ...
      TELEGRAM_FIELD_NAMES.every(
        (f) => telegramChallengesConfig[0][f.field] !== undefined,
      )
    ) {
      const valObj = {};
      TELEGRAM_FIELD_NAMES.forEach((f) => {
        valObj[f.field] = String(telegramChallengesConfig[0][f.field]);
      });
  ...
  const isTelegramFormValid =
    ...
    TELEGRAM_FIELD_NAMES.some(
      (f) => telegramInputs[f.field] !== telegramOriginalInputs[f.field],
    ) &&
  ...
  function handleUpdateTelegramConfig() {
    ...
    const postData = {
      ...configToSend,
      id:
        telegramChallengesConfig && telegramChallengesConfig.length > 0
          ? telegramChallengesConfig[0].id
          : null,
    };
  ...
  function handleUpdateConfig() {
    ...
    const postData = {
      ...configToSend,
      id:
        surfingConfigs && surfingConfigs.length > 0
          ? surfingConfigs[0].id
          : null,
    };
          ...
          {TELEGRAM_FIELD_NAMES.map(({ label, field }) => (
            <>
              ...
              <input
                ...
                onChange={(e) =>
                  handleTelegramInputChange(field, e.target.value)
                }
              />
            </>
          ))}
