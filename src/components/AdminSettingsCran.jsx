...
export default function AdminSettingsCran({
  initialNumbers,
  setInitialNumbers,
  course,
  setCourse,
  ...
  const isFormValid =
    inputs.some((val, idx) => val !== originalInputs[idx]) &&
    inputs.every((val) => val !== "0" && val !== "");
  ...
  const handleUpdateCourse = () => {
    ...
    api
      .post("/api/freetonadmin/course", postData)
      .then((response) => {
  ...
  const handleUpdatePrizeTable = () => {
    ...
    api
      .post("/api/freetonadmin/prizestable", postData)
      .then((response) => {
  ...
  return (
    <div className="cran-settings-container">
      <div className="create-ad-package-container">
        <div className="cran-settings-form-title">РЕДАКТИРОВАНИЕ ПРИЗОВ</div>
