...
export default function Friends({ friends, starsMode, course }) {
    ...
    return friends.map((friend, index) => (
      <div className="transaction-row" key={friend.id || index}>
        ...
        <div className="transaction-cell">
          {t("friends.income")}:{" "}
          {starsMode
            ? (friend.prize * course).toFixed(6)
            : friend.prize.toFixed(6)}
        </div>
      </div>
    ));
  };
  ...
  return (
    <div className="friends-container">
      ...
      <button className="friends-button-copy" onClick={writeLinkInClipboard}>
        {t("friends.copyLink")}
      </button>
      ...
      <div className="friends-balance-container">
        ...
        <div className="friends-balance-value">
          {starsMode ? (totalPrize * course).toFixed(6) : totalPrize.toFixed(6)}
        </div>
      ...
      <div className="friends-list">{renderFriendsTable()}</div>
    </div>
  );
}
